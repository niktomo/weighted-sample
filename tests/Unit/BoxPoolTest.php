...
class BoxPoolTest extends TestCase
{
    ...
    public function test_throws_on_empty_items(): void
    {
        ...
        // Act
        BoxPool::of([], static fn (array $item): int => $item['weight'], static fn (array $item): int => $item['count']);
    }
    ...
    public function test_zero_count_items_are_excluded_by_default(): void
    {
        ...
        $pool = BoxPool::of($items, static fn (array $item): int => $item['weight'], static fn (array $item): int => $item['count'], randomizer: $this->fixedRandomizer(0));
    ...
    public function test_throws_when_all_items_are_filtered_out(): void
    {
        ...
        // Act
        BoxPool::of($items, static fn (array $item): int => $item['weight'], static fn (array $item): int => $item['count']);
    }
    ...
    public function test_strict_filter_throws_on_zero_count(): void
    {
        ...
        // Act
        BoxPool::of($items, static fn (array $item): int => $item['weight'], static fn (array $item): int => $item['count'], filter: new StrictValueFilter());
    }
    ...
    public function test_of_accepts_generator_as_items(): void
    {
        ...
        // Act
        $pool = BoxPool::of($generator, static fn (array $item): int => $item['weight'], static fn (array $item): int => $item['count']);
