...
| `AliasTableSelector` | O(n) | O(1) | 整数 pick（構築時のみ内部で浮動小数点を使用） |
...
エイリアス法は整数演算1回の乱数呼び出しで O(1) 抽選を実現します。アイテム数に関係なく
一定速度です。アイテム数が多い（目安: 50件以上）プールで頻繁に `draw()` を呼ぶ場合に最適。
...
アイテム数   PrefixSum O(log n)   Alias O(1)   倍率
    10             ~194 ms          ~96 ms      2.0倍速
    50             ~253 ms          ~99 ms      2.6倍速
   100             ~270 ms          ~98 ms      2.8倍速
   200             ~284 ms         ~100 ms      2.8倍速
   500             ~316 ms         ~100 ms      3.2倍速
  1000             ~328 ms         ~102 ms      3.2倍速
```

Alias の抽選スループットはアイテム数に関わらず ~100 ms で一定（真の O(1)）。
