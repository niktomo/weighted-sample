...
// static fn (array $item): int => 重み,  static fn (array $item): int => 在庫数
$pool = BoxPool::of(
    $items,
    static fn (array $item): int => $item['weight'],  // 抽選確率
    static fn (array $item): int => $item['stock'],   // 何回抽選できるか
);
```
...
- `BoxPool` は**ミュータブル設計**です。`draw()` のたびに内部状態が変化します。並行コンテキストで共有する場合は外部での同期が必要です。
...
>
> **624 出力が危険な理由：** Mt19937 は 624 要素の内部状態を持ちます。攻撃者が連続した 624 個の
> 32bit 出力（例：API レスポンスに含まれる抽選結果）を収集すると、MT19937 状態復元アルゴリズムに
> より内部状態を完全に再構築でき、以降の出力をすべて予測できます。生の index ではなくアイテムの
> レアリティ等（部分情報）であっても、探索空間を大幅に絞り込めます。
>
...
```php
// エンティティごとに再現可能なシードを導出する。
// 同じ userId + campaignId は常に同じ抽選列を生成する：
$seed = (int) hexdec(substr(hash('sha256', $userId . ':' . $campaignId), 0, 8));

// 一回限りの再現可能な抽選列が必要な場合は、シードを生成して永続化し、
// 後で同じ抽選列を再現するために再利用する：
$seed = unpack('N', random_bytes(4))[1]; // この値を保存しておく — 再現の鍵になる
// 注意: 再現が不要なら SecureRandomizer（デフォルト）を使用してください。
```
...
`FenwickTreeSelector` は**ミュータブル**です。`subtract()` のたびに内部 Fenwick ツリーをインプレースで更新します。これは意図的な設計で、インプレース変更によってオブジェクト生成なしに O(log n) 更新を実現し、BoxPool を大規模でも効率的にします。
